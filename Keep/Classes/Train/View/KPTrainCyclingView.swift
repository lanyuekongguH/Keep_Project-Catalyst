...
class KPTrainCyclingView: UIScrollView {
    ...
    fileprivate lazy var cyclingButton: UIButton = {
        ...
        cyclingButton.setBackgroundImage(UIImage.createImage(KPGreen()), for: UIControl.State())
        cyclingButton.setBackgroundImage(UIImage.createImage(KPGreen()), for: .highlighted)
        cyclingButton.setTitle("出发去骑行", for: UIControl.State())
        cyclingButton.setTitleColor(UIColor.white, for: UIControl.State())
